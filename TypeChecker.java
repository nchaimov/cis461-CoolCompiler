...
public class TypeChecker {
	...
	public boolean typecheck() {
		try {
			...
			if (!env.classes.containsKey("Main")) {
				...
				final Environment.CoolMethod mainMethod = env.classes.get("Main").methods
						.get("main");
				if (mainMethod == null) {
	...
	public void identifyClasses(final ASTnode node) throws Environment.EnvironmentException,
			TypeCheckException {
		if (node != null) {
			switch (node.kind) {
			case (sym.CLASS):
	...
	public void identifyParents(final ASTnode node) throws Environment.EnvironmentException,
			TypeCheckException {
		if (node != null) {
			switch (node.kind) {
			case (sym.CLASS):
				if (node.left != null && node.left.kind == sym.INHERITS) {
					final Environment.CoolClass thisClass = env.getClass((String) node.value);
	...
	protected void getMethodsAndAttributes() throws TypeCheckException,
		...
		for (final Entry<String, Environment.CoolClass> e : env.classes.entrySet()) {
	...
	protected void getMethodsAndAttributes(final Environment.CoolClass curClass, final ASTnode node)
		...
		if (node != null) {
			...
			case Nodes.ATTRIBUTE: {
				...
				final Environment.CoolAttribute attr = new Environment.CoolAttribute(name, type);
				attr.node = node;
			...
			case Nodes.METHOD: {
				...
				final Environment.CoolMethod method = new Environment.CoolMethod(name, returnType);
				method.node = node;
				if (node.center != null) {
	...
	private void processMethodArguments(final Environment.CoolMethod method, final ASTnode node)
			throws Environment.EnvironmentException, TypeCheckException {
		if (node != null) {
			switch (node.kind) {
			case sym.COLON: {
				final String name = (String) node.left.value;
				final Environment.CoolClass type = env.getClass((String) node.right.value);
	...
	public void checkAttributes() throws Environment.EnvironmentException, TypeCheckException {
		for (final Entry<String, Environment.CoolClass> e : env.classes.entrySet()) {
			...
			for (final Entry<String, Environment.CoolAttribute> e2 : curClass.attributes.entrySet()) {
	...
	public void checkMethods() throws Environment.EnvironmentException, TypeCheckException {
		for (final Entry<String, Environment.CoolClass> e : env.classes.entrySet()) {
			...
			for (final Entry<String, Environment.CoolMethod> e2 : curClass.methods.entrySet()) {
	...
	public Environment.CoolClass check(final Environment.CoolClass curClass, final ASTnode node)
			throws Environment.EnvironmentException, TypeCheckException {
		if (node != null) {
			switch (node.kind) {
			...
			case sym.ASSIGN: {
				...
				final Environment.CoolClass leftType = check(curClass, node.left);
				final Environment.CoolClass rightType = check(curClass, node.right);
				log(MessageFormat.format(
			...
			case sym.DOT: {
				...
				final Iterator<Environment.CoolClass> actualIter = actuals.iterator();
				final Iterator<Environment.CoolAttribute> formalIter = formals.iterator();

				while (actualIter.hasNext() && formalIter.hasNext()) {
					final Environment.CoolClass expectedType = formalIter.next().type;
			...
			case sym.IF: {
				...
				final Environment.CoolClass unionType = mostSpecificParent(node.center.type,
						node.right.type);
	...
	private List<Environment.CoolClass> getCaseTypes(final Environment.CoolClass curClass,
			final ASTnode node, final List<Environment.CoolClass> list)
			throws Environment.EnvironmentException, TypeCheckException {
		if (node != null) {
	...
	private int addLetIntroductions(final Environment.CoolClass curClass, final ASTnode node,
			int numVars) throws Environment.EnvironmentException, TypeCheckException {
	...
	private Environment.CoolClass checkSequence(final Environment.CoolClass curClass,
			final ASTnode node) throws Environment.EnvironmentException, TypeCheckException {
		if (node != null) {
			if (node.kind == sym.SEMI) {
				final Environment.CoolClass leftClass = checkSequence(curClass, node.left);
				final Environment.CoolClass rightClass = checkSequence(curClass, node.right);
				if (rightClass != null)
					return rightClass;
				else
					return leftClass;
	...
	private void typecheckMethodArguments(final Environment.CoolClass curClass, final ASTnode node)
			throws Environment.EnvironmentException, TypeCheckException {
		if (node != null) {
			if (node.kind == sym.COMMA) {
	...
	protected boolean moreGeneralOrEqualTo(final Environment.CoolClass c1, Environment.CoolClass c2)
			throws Environment.EnvironmentException {
