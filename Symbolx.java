 ...
 *  @author Michal Young
 */
class Symbolx extends java_cup.runtime.Symbol {

    /* Inherited fields from CUP's Symbol class are
       sym:    the symbol type
       ...
       It is common to repurpose the "left" and "right" fields to be 
       line number and column number, but this makes nonsense of the way
       CUP identifies the position of non-termminal symbols in error 
       messages.  So, instead, we keep the left and right character 
       positions as they are, and add a line# and col# field. 

       To accomodate #include and similar constructs that are 
       handled in the lexer, we'll also have a place to put 
       the file name, bt it's optional. 
    */ 
    ...
    /* Now we need a constructor to fill this information in ... 
       and since we are trying to keep the left and right values, 
       we can't override the constructor Symbol(id, l, r, val).
       We'll need a constructor with lots of arguments, alas. 
    */

    public Symbolx(int id, int _left, int _right, 
		  int _line, int _col, 
		  Object _val) {
